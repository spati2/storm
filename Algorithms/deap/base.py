...
class Fitness(object):
    ...
    def dominates(self, other, obj=slice(None)):
        ...
        if self.feasible and other.feasible: return self.normal_dom(other,obj)
            ...
            self.overages = self.problem.evalConstraintOverages(self.decisionValues)
            other.overages = other.problem.evalConstraintOverages(other.decisionValues)
            if sum(self.overages) > sum(other.overages): return False
            else: return True
        
    def normal_dom(self, other, obj=slice(None)):
        not_equal = False
